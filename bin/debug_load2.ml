...
let length{n: nat}(v: vec(n)): single_nat(n) =
  match v with
  { vnil => single_zero
  ; vcons{m}(_)(vp) => single_succ{m}(length{m}(vp))
  }

let to_int{n: nat}(k: single_nat(n)): int =
  match k with
  { single_zero => 0
  ; single_succ{m}(kp) => 1 + to_int{m}(kp)
  }

let main: int =
  let v2 = vcons{succ(zero)}(10)(vcons{zero}(20)(vnil)) in
  to_int{succ(succ(zero))}(length{succ(succ(zero))}(v2))
"

let () = 
  Compile.debug_store := true;
  Compile.debug_subst := true;
  print_endline "=== Testing length -> to_int ===";
  match (
    ...
    List.iter (fun c -> print_endline (Sequent.Aarch64.Code.Code.to_string c)) asm;
    print_endline "=== Running ===";
    let* result = Pipe.EmitStage.eval ~trace:false ~max_steps:50000 asm in
