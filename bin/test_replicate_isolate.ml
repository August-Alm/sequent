(* Test to isolate replicate issue - compare recursive vs manual cons *)
module Pipe = Sequent.Pipeline

(* Test A: Does replicate(1) return cons with correct structure? *)
let sourceA = {|
data nat: type where
  { zero: nat
  ; succ: nat -> nat
...
let main: int =
  let n1 = single_succ{zero}(single_zero) in
  let v = replicate{int}{succ(zero)}(42)(n1) in
  length{int}{succ(zero)}(v)
|}

(* Test B: Simpler - just check if replicate(2)'s head element is correct *)
let sourceB = {|
data nat: type where
  { zero: nat
  ; succ: nat -> nat
...
let head{a}{n: nat}(v: vec(a)(succ(n))): a =
  match v with
  { cons{_}{m}(x)(xs) => x
  }
...
let run_test name source expected =
  ...
  let result =
    ...
    let* result = Pipe.EmitStage.eval ~trace:false ~max_steps:10000 code in
    Ok result
  in
  match result with
  | Ok actual -> 
...
let () =
  run_test "replicate(1) length" sourceA 1;
  run_test "replicate(2) head" sourceB 42
