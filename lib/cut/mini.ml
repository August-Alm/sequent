...
let cont_sym = Path.of_primitive 300 "cont"
let ret_sym = Path.of_primitive 301 "ret"
let ret_method =
  let a = Ident.mk "a" in
  let x = Ident.mk "x" in
  { Types.parent = cont_sym
  ; Types.symbol = ret_sym
  ; Types.quantified = [(a, Types.KStar)]
  ; Types.environment = [ (x, Types.Cns (Types.TyVar a)) ]
  ; Types.result_type = Types.Cns (Types.TyApp (Types.TySym cont_sym, Types.TyVar a))
  ; Types.constraints = None
  }
let cont_signature =
  { Types.symbol = cont_sym
  ; Types.parameters = [(None, Types.KStar)]
  ; Types.methods = [ret_method]
  }
...
module type CONS = sig
  ...
  val head: t
  val tail: t list
  module L: LIST with type t = t
end

module Nil (T: TYPE) : LIST with type t = T.t = struct
  type t = T.t
  let values = []
end

module Cons (V: VALUE) (L: LIST with type t = V.t) : CONS with type t = V.t = struct
  type t = V.t
  let head = V.value
  let tail = L.values
  module L = struct
...
module Append (L1: LIST) (L2: LIST with type t = L1.t) : LIST with type t = L1.t = struct
  type t = L1.t
  let values = L1.values @ L2.values
end

type (_,'t) name =
  | Z : (module CONS with type t = 't) * 't -> ((module CONS with type t = 't), 't) name
  | S : ('n, 't) name -> ((module CONS with type t = 't), 't) name