...
let lookup_int (e: linear_env) (x: var) : int64 =
...
let step ((cmd, e): config) : config option =
  ...
  | Add (v1, v2, v, body) ->
      (match (lookup_opt e.values v1, lookup_opt e.values v2) with
        (Some (IntVal n1), Some (IntVal n2)) ->
          let new_values = push e.values v (IntVal (Int64.add n1 n2)) in
          Some (body, { e with values = new_values })
      | (Some val1, Some val2) ->
  ...
  | Sub (v1, v2, v, body) ->
      (match (lookup_opt e.values v1, lookup_opt e.values v2) with
        (Some (IntVal n1), Some (IntVal n2)) ->
          let new_values = push e.values v (IntVal (Int64.sub n1 n2)) in
          Some (body, { e with values = new_values })
      | (Some val1, Some val2) ->
  ...
  | Mul (v1, v2, v, body) ->
      (match (lookup_opt e.values v1, lookup_opt e.values v2) with
        (Some (IntVal n1), Some (IntVal n2)) ->
          let new_values = push e.values v (IntVal (Int64.mul n1 n2)) in
          Some (body, { e with values = new_values })
      | (Some val1, Some val2) ->
  ...
  | Div (v1, v2, v, body) ->
      (match (lookup_opt e.values v1, lookup_opt e.values v2) with
        (Some (IntVal n1), Some (IntVal n2)) ->
          let new_values = push e.values v (IntVal (Int64.div n1 n2)) in
          Some (body, { e with values = new_values })
      | (Some val1, Some val2) ->
  ...
  | Rem (v1, v2, v, body) ->
      (match (lookup_opt e.values v1, lookup_opt e.values v2) with
        (Some (IntVal n1), Some (IntVal n2)) ->
          let new_values = push e.values v (IntVal (Int64.rem n1 n2)) in
          Some (body, { e with values = new_values })
      | (Some val1, Some val2) ->
...
let rec run ?(trace=false) ?(steps=0) ?(max_steps=500) (cfg: config) : config * int =
  ...
  if trace then begin
    ...
    (match cmd with
      Ifz (v, _, _) -> 
        (try Printf.printf " [ifz val = %s]" (Int64.to_string (lookup_int e.values v))
        with _ -> Printf.printf " [ifz val = ?]")
    | Add (a, b, _, _) ->
        (try Printf.printf " [%s + %s]" (Int64.to_string (lookup_int e.values a))
                                        (Int64.to_string (lookup_int e.values b))
        with _ -> Printf.printf " [? + ?]")
    | Axiom (_, v, _) ->
        (try Printf.printf " [val = %s]" (Int64.to_string (lookup_int e.values v))
        with _ -> ())
    | _ -> ());
    Printf.printf "\n"
  end;
