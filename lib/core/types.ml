...
(* Types *)
type typ =
    Pos (* Kind of positive types *)
  | Neg (* Kind of negative types *)
  | Arrow of typ * typ (* Only for kinds *)
  | Int (* Primitive type of integers; is positive *)
  | TVar of Ident.t
  | TMeta of Ident.t
  | Sgn of Path.t * typ list (* Applied data or codata type *)
  | PromotedCtor of Path.t * Path.t * typ list

let as_typ = function Positive -> Pos | Negative -> Neg

type chiral_typ = Prd of typ | Cns of typ

let strip_chirality = function Prd t | Cns t -> t

let chiral_map f = function Prd t -> Prd (f t) | Cns t -> Cns (f t)
...
(* A constructor is promotable (can be lifted to the type/kind level) if:
  ...
  3. Its result type is the canonical form: Sgn(typeName, [TVar q1, TVar q2, ...])
...
let is_promotable type_name (ctor: xtor) : bool =
  ...
  let only_producer_args = List.for_all (
    function Prd _ -> true | Cns _ -> false
...
let is_dec_promotable (dec: dec) : bool =
  dec.polarity = Positive && (* Only data types are promotable *)
...
let rec infer_kind (ctx: context) (t: typ) : (typ, kind_error) result =
  ...
  | Arrow (t1, t2) ->
      ...
      Ok Neg (* Arrow types are negative *)
  | Sgn (name, args) ->
...
let data_kind_from_param_kinds (param_kinds: typ list) : typ =
  List.fold_right (fun k acc -> Arrow (k, acc)) param_kinds Pos
