...
module Convert: sig
  ...
  let rec to_sc_typ (t: typ) : CT.typ =
    match t with
    | TySym s -> CT.TySym s
    | TyVar x -> CT.TyVar x
    | TyApp (t1, t2) -> CT.TyApp (to_sc_typ t1, to_sc_typ t2)
    | TyFun (t1, t2) ->
      CT.TyApp (CT.TyApp (CT.Prim.fun_typ, to_sc_typ t1), to_sc_typ t2) 
    | TyAll ((x, k), t) ->
      CT.TyApp (CT.Prim.all_typ x k, to_sc_typ t)
    | TyDef (Prim (s, k)) ->
      CT.TyDef (CT.Prim (s, k))
    | TyDef (Data td) ->
      CT.TyDef (CT.Data (data_to_sc td))
    | TyDef (Code td) ->
      CT.TyDef (CT.Code (code_to_sc td))
  
  and data_to_sc (td: ty_dec) : CT.ty_dec =
    { CT.symbol = td.symbol
    ; CT.arguments = List.map (fun (t_opt, k) ->
        (Option.map to_sc_typ t_opt, k)
      ) td.arguments
    ; CT.xtors = List.map ctor_to_sc td.xtors
    }

  and ctor_to_sc (ctor: ty_xtor) : CT.ty_xtor =
    { CT.parent = ctor.parent
    ; CT.symbol = ctor.symbol
    ; CT.quantified = List.map (fun (x, k) -> (x, k)) ctor.quantified
    ; CT.producers = List.map to_sc_typ ctor.sources
    ; CT.consumers = []
    ; CT.arguments = List.map to_sc_typ ctor.arguments
    ; CT.constraints = Option.map (List.map (fun (x, t) -> (x, to_sc_typ t))) ctor.constraints
    }
  
  and code_to_sc (td: ty_dec) : CT.ty_dec =
    { CT.symbol = td.symbol
    ; CT.arguments = List.map (fun (t_opt, k) ->
        (Option.map to_sc_typ t_opt, k)
      ) td.arguments
    ; CT.xtors = List.map dtor_to_sc td.xtors
    }

  and dtor_to_sc (dtor: ty_xtor) : CT.ty_xtor =
    let srcs, c =
